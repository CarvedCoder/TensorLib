...
TEST(TensorTest, MoveSemantics) {
    ...
    Tensor t1 = Tensor::CreateTensor(std::move(arr), 6, {2, 3});
    Tensor t2 = std::move(t1);

    // t2 must have valid data
    EXPECT_FLOAT_EQ(t2.getDataPtr()[0], 0.0f);

    // moved-from object must be safe to destroy
    // DO NOT access t1.getDataPtr()
}
