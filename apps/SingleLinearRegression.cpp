...
int main(int argc, char* argv[]) {
    ...
    auto data_path = "../csv/Salary_dataset.csv";
    DataFrame df(CSVParser::readCSV(data_path));
    df.drop("");
    auto csv_data = df.toNumeric();
    Tensor X = DataLoader::toTensor(csv_data, "YearsExperience");
    Tensor y = DataLoader::toTensor(csv_data, "Salary");
    Tensor w = Tensor::createRandTensor({1});
    Tensor b = Tensor::createRandTensor({1});
    Tensor y_hat = Tensor::createZeros({X.getShape()});
    std::println("Initial prediction for {} years of experience is {}", X(15), w(0) * X(15) + b(0));
    std::println("Now using LinearRegression");
    LinearRegression(X, y, w, b, y_hat, 0.01, 1000);
    std::println("After training prediction for {} years of experience is {}", X(15),
                 w(0) * X(15) + b(0));
    std::println("Difference is {}", y(15) - y_hat(15));
    return 0;
}
